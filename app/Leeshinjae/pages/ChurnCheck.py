...
def prepare_input_data(age, gender, country, sub_type, device, listening_time, 
                      songs_per_day, skip_rate, ads_listened, offline):
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            'steps': [
                ...
                {'range': [threshold * 100 * 0.7, threshold * 100], 'color': "#3a3a1a"},
...
country = st.sidebar.selectbox("국가", ["US", "UK", "DE", "FR", "CA", "IN"], key="country")
sub_type = st.sidebar.selectbox("구독 유형", ["Free", "Premium", "Family", "Student"], key="sub_type")
...
listening_time = st.sidebar.slider("하루 청취 시간 (분)", 0.0, 180.0, 60.0, key="listening_time")
...
input_data = prepare_input_data(
    age, gender, country, sub_type, device,
    listening_time, songs_per_day, skip_rate, ads_listened, offline
)
...
if ml_prob is not None and dl_prob is not None:
    ...
    with cols[0]:
        ...
        """, unsafe_allow_html=True)
        fig_ml = create_gauge_chart(ml_prob, best_ml_threshold, f"{best_ml_name} 예측", "#1DB954")
    ...
    with cols[1]:
        ...
        """, unsafe_allow_html=True)
        fig_dl = create_gauge_chart(dl_prob, dl_threshold, "Deep Learning 예측", "#00d4ff")
    ...
    comparison_fig = create_comparison_chart(ml_prob, dl_prob, best_ml_threshold, dl_threshold)
    ...
    with col_avg2:
        # 아이콘을 뺀 텍스트만 추출하여 표시
        clean_risk_text = avg_risk_text.replace("🚨 ", "").replace("⚠️ ", "").replace("✅ ", "")
        st.markdown(f"""
            <div class="summary-metric-container">
    ...
    # 인사이트 생성 및 표시
    insights = generate_insights(input_data, ml_prob, dl_prob, best_ml_threshold, dl_threshold)
    ...
    summary_data = {
        '항목': ['나이', '성별', '국가', '구독 유형', '기기', '청취 시간(분)', '일일 재생 곡 수', 
                '스킵 비율', '주간 광고 수', '오프라인 사용'],
        '값': [age, gender, country, sub_type, device, listening_time, songs_per_day,
              f"{skip_rate*100:.1f}%", ads_listened, "예" if offline else "아니오"]
    }
